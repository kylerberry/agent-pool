...
## 0. Current phase: Pool Proof complete; functional deployment active under simplified local governance
...
The active build phase is the direct-task-first functional deployment in `raw/specs/functional-pool-deployment.md`, with 17 ADR-018 nodes. `raw/plans/proposed-build-dag.json` is the authorized canonical plan under the simplified local governance in `raw/context/local-repository-builder-workflow.md`: its embedded human-attributed `approval` object plus the ledger's frozen plan SHA govern local dispatch. The detached exact candidate/source/scope/archive approval Kyler recorded on 2026-08-15 (`raw/plans/functional-pool-deployment-approval.json`) is retained historical evidence of that activation, not an ongoing dispatch requirement. The sole ready root is `deployment-bootstrap-policy-and-glm52-qualification`; do not reserve or start it without a separate explicit instruction.

An unapproved four-node replacement milestone candidate exists at `raw/plans/replacement-milestone-dag.candidate.json` (generalize proven runner → compose direct intake to execution + general deterministic verifier → surface reviewable output). It is not active; activating it requires a separate future approval and operation.
