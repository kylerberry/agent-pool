...
The active build plan is the four-node replacement milestone at `raw/plans/proposed-build-dag.json` (generalize-proven-runner → compose-direct-intake-to-execution + general-deterministic-verifier → surface-reviewable-output), approved by Kyler on 2026-08-16 under the simplified local governance in `raw/context/local-repository-builder-workflow.md`: the plan's embedded human-attributed `approval` object plus the ledger's frozen plan SHA govern local dispatch. The plan is a tracer path to the first usable pool — a real-repository runner, direct-task intake through a SQLite-backed claim loop to a fresh Worker, a general deterministic verifier, and human-reviewable local output. Redis/BullMQ, Tier-2 grading, GitHub automation, and the full governance stack remain deferred until observed need. The sole ready root is `generalize-proven-runner`; do not reserve or start it without a separate explicit instruction.
