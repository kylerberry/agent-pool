...
## 0. Current phase: replacement milestone plan active under simplified local governance
...
The active build plan is the four-node repository-bound pool milestone at `raw/plans/proposed-build-dag.json` (configure-repository-bound-pool → compose-direct-intake-to-execution + general-deterministic-verifier → surface-reviewable-output), approved by Kyler on 2026-08-20 under the simplified local governance in `raw/context/local-repository-builder-workflow.md`: the plan's embedded human-attributed `approval` object plus the ledger's frozen plan SHA govern local dispatch. The passed generalized-runner evidence is preserved in the archived prior run and is reused as a code dependency, not reopened as a node. The plan is a tracer path to the first usable one-repository-per-pool instance: owner-supplied repository policy, task-only direct intake through a SQLite-backed claim loop to a fresh Worker, a general deterministic verifier, and host-accessible local review output. Redis/BullMQ, Tier-2 grading, multi-repository routing, GitHub automation, and the full governance stack remain deferred until observed need. The sole ready root is `configure-repository-bound-pool`; do not reserve or start it without a separate explicit instruction.

The superseded 17-node functional deployment plan (approved 2026-08-15; no node started) is archived byte-for-byte at `raw/plans/superseded-functional-deployment-build-dag.json`; `raw/specs/functional-pool-deployment.md` and the detached exact-hash approval artifacts are retained historical evidence, not dispatch authority.
...
- Pin and preflight Pool Worker capabilities from `packages/worker-harness/config/runtime-versions.json`. The approved target scope is seven exact models: Luna/Terra/Sol, GLM-5.2/GLM-5.3, and Kimi K2.7 Code/Kimi K3. Z.ai models remain ineligible until qualified; Moonshot is fallback-only; do not select Anthropic or any unlisted model.
