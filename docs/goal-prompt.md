...
The active build plan is the four-node replacement milestone at `raw/plans/proposed-build-dag.json`. `generalize-proven-runner` and `compose-direct-intake-to-execution` are complete. Remaining nodes: `general-deterministic-verifier` and `surface-reviewable-output`. Redis/BullMQ, Tier-2 grading, GitHub automation, and the full governance stack remain deferred until observed need.
