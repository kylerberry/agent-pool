...
### 4.3 Dispatch (ADR-010, ADR-019)

Each round the controller computes the **ready frontier** (all dependencies `passed`) and applies controller-owned predicted-touch constraints derived from the Gate 1 code-graph snapshot. Ready nodes with likely shared surfaces are serialized without rewriting their approved semantic DAG edges; low-confidence or unavailable predictions fall back to optimistic concurrency. The controller enqueues the remaining self-contained jobs, pool concurrency limits throttle actual parallelism, results update node state, and the frontier is recomputed until terminal. Integration re-verification remains the final conflict arbiter.
...
- **Retry ceilings, per failure class (ADR-023):** attempt failures are classified `logic` (the node's own defect) or `integration` (a previously-green node re-failed at re-verification because a sibling changed the head/suite). Each class has its own fixed ceiling (default 3; overrides downward only). A node is never escalated as defective for losing integration races — the escalation record names the class, since the two imply different human resolutions. Predicted and actual touched graph units, serialization decisions, and integration outcomes are recorded for calibration. Repeated actual integration failures or sustained prediction misses may recommend amend-DAG (§4.5), but predictions never mutate topology.
...
Escalations surface as audit-trail queries (CLI / minimal dashboard) — no push-notification infrastructure in v1. Five fixed, logged resolution actions: **retry** (optional edit; counter resets), **manual fix**, **cancel branch**, **override/force-pass** (logged reason mandatory; flagged distinctly as a machine-gate bypass), and **amend-DAG** (ADR-024) — human-approved partial re-decomposition when implementation reveals a planned boundary is impossible. The controller may recommend amend-DAG after a versioned contention threshold, but never executes it automatically or from predicted overlap alone. Amendment cancels the affected subtree, re-decomposes only the unmet remainder (original spec slice + already-passed nodes), validates mechanically, derives and freezes predicted-touch metadata from the current code-graph snapshot with the proposal, and re-approves the combined proposal through Gate 1. Passed work is never discarded. The quarantine principle restated precisely: the DAG never changes *silently* — every topology change is proposed, validated, human-approved, and logged.
...
7. **Failure context survives compaction (ADR-026)** — a failing phase's artifact must carry what was attempted, why it failed, and discoveries made; retries receive prior failure artifacts in their payload and never start blind. Retained transcripts are finalized, redacted, hashed, persisted and verified outside the ephemeral workspace, then transactionally indexed by durable object id before cleanup. They remain an authorized-human escape hatch and are never injected into prompts. Extraction failure marks the attempt `audit_incomplete` without replacing the structured failure artifact or allowing indefinite workspace retention.
...
Decomposer vs. C on the same knowledge: **breadth vs. depth** — the decomposer queries across units to draw boundaries; C queries within one unit to plan it. At Gate 1 the controller freezes versioned predicted-touch metadata derived from the code graph, then uses likely overlap for scheduling serialization while measuring predicted versus actual touched units and integration contention. Prediction is advisory; it neither rewrites semantic dependencies nor replaces integration re-verification. Vector embeddings are consciously absent (nothing in this system has the un-compiled-raw-corpus shape they serve); LLM-wiki failure modes accepted as obligations: S-phase lint/audit pass against sources; directory indexes past ~200 files.

## 11. Storage & Audit (ADR-014)

**SQLite**, single file, orchestrator-owned—justified by ADR-001 itself (one deterministic controller process = one writer). Holds DAG/node state, state versions, unique attempt/result records, models, evidence, findings, cost, suite hashes, escalations/resolutions, routing results, predicted/actual contention evidence, and transcript-object metadata/audit-completeness state. Transcript bytes live in bounded-retention object storage, not ephemeral workspaces or SQLite; the index stores durable object identity, hash, size, schema/redaction/access metadata, and lifecycle status. Redis owns queue/session records. ADR-031 defines the practical cross-store idempotency and reconciliation contract. Multi-tenant scale-out remains a future deployment variant.
