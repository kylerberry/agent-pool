...
- Startup validates model scope against the active canonical constant, not agreement between mutable config files. The current implementation remains the legacy five-model gate. The proposed seven-model target (Luna/Terra/Sol, GLM-5.2/GLM-5.3, Kimi K2.7 Code/Kimi K3) becomes active only after approved-plan activation and real Z.ai qualification.
