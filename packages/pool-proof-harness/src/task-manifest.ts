/**
 * Strict validator for reviewed task manifests (generalize-proven-runner).
 *
 * The manifest file is untrusted input at the harness trust boundary. Every
 * structural bound below must pass before the runner performs preflight,
 * clone, sandbox, or any model work. The validator executes no git or docker
 * process; the only filesystem access is resolving `target_repo_path` (one
 * isolated function, so tests inject temp paths).
 */

import { createHash } from 'node:crypto';
import { readFileSync, realpathSync, statSync } from 'node:fs';
import { isAbsolute } from 'node:path';
import { isApprovedModelId, type ApprovedModelId } from '../../../src/domains/model-routing-and-evaluation/approved-models.ts';
