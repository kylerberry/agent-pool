...
5. Uses one foreground Pi `subagent` call at a time (`agent: "local-craft-*"`, `context: "fresh"`); phases are not combined into a parallel chain. A non-empty C `security_triggers` list adds one sequential supplemental plan-security call after the C planner and before R.
...
C emits `phase_data.security_triggers` from the contract's closed vocabulary. An empty list keeps the normal flow; any trigger deterministically requires elevated controls. C does not assign `low|medium|high` scores or produce separate asset/abuse-case documents. It uses the existing `trust_boundaries` and `test_strategy` fields for the concrete plan. Unknown or duplicate triggers fail validation.

For triggered work, the conductor invokes `local-craft-security` **after C and before R** in a fresh, independent context. It applies `security-and-hardening` to the plan, original criteria, declared triggers, trust boundaries, and test strategy. This is a supplemental C checkpoint, not a new CRAFTS phase or a T artifact. Blocking plan findings return to C; Render begins only after the plan-security report passes. Pass that compact report to R, T, and S. Do not add this checkpoint when `security_triggers` is empty.

### R — Render (Test-Drive)

Write failing tests first, then implement the minimum change to pass, then refactor.

Use the Pi `subagent` tool with `agent: "local-craft-builder"` and `context: "fresh"` for this phase. Pass the C phase report and the original acceptance criteria; for triggered work, also pass the passed plan-security report. Do not begin implementation until that report passes. The fresh local builder child edits the assigned workspace and returns the R-phase artifact; the local conductor only gates phases and validates/forwards artifacts. The conductor does not implement in the parent context.
...
Use the Pi `subagent` tool with `agent: "local-craft-security"` and `context: "fresh"` for this phase. Pass the task goal, changed files, verification output, and any trust boundaries identified during Conceptualize or Render; for triggered work, also pass C's declared security triggers and plan-security report.
...
- Escalate lite work to the full flow before editing when any closed-vocabulary security trigger applies, so C can emit it and the conductor can run the independent plan-security checkpoint.
...
- Replaced subjective risk classification with closed-vocabulary `security_triggers`; any trigger requires the fresh-context plan-security checkpoint while an empty list preserves the normal flow.
