...
export const DOMAIN_MAP_APPROVAL_RELATIVE_PATH = "docs/raw/plans/domain-map-approval.json";
...
const PLAN_BOUNDS = {
  ...
  max_scope_review_bytes: 1024 * 1024,
...
function safeRelativePath(value, label) {
  if (typeof value !== "string" || !value || value.length > PLAN_BOUNDS.max_string_field) fail(`${label} is invalid`);
  if (path.isAbsolute(value) || value.split("/").some((segment) => !segment || segment === "." || segment === "..")) fail(`${label} must be a repository-relative path`);
  return value;
}
...
export function validatePlanObject(plan, byteLength) {
  ...
  exactKeysOptional(plan, ["schema_version", "nodes", "approval"], ["kind", "source", "domain_boundaries_changed", "scope_review_path"], "plan");
  ...
  // Plan-level domain signal. Absence (legacy plans) means no boundary change; only an explicit
  // boolean true conditionally activates domain-map approval and hash validation.
  if (plan.domain_boundaries_changed !== undefined && typeof plan.domain_boundaries_changed !== "boolean") {
    fail("domain_boundaries_changed must be a boolean");
  }
  if (plan.scope_review_path !== undefined) safeRelativePath(plan.scope_review_path, "scope_review_path");
  ...
  if (typeof plan.approval.approved_by !== "string" || !plan.approval.approved_by.trim() || plan.approval.approved_by.length > PLAN_BOUNDS.max_approver_length) {
  ...
  for (const node of plan.nodes) {
    ...
    if (node.acceptance_criteria.length > PLAN_BOUNDS.max_criteria) fail(`node ${node.id} has too many criteria`);
...
function validateDomainMapApproval(root) {
  const { value: approval } = readVerifiedRepositoryJson(root, DOMAIN_MAP_APPROVAL_RELATIVE_PATH, { label: "domain-map approval" });
  exactKeys(approval, ["map_path", "map_sha256", "approved_by", "approved_at"], "domain-map approval");
  safeRelativePath(approval.map_path, "domain-map approval map_path");
  if (!SHA256_PATTERN.test(approval.map_sha256)) fail("domain-map approval map_sha256 is invalid");
  if (typeof approval.approved_by !== "string" || !approval.approved_by.trim() || approval.approved_by.length > PLAN_BOUNDS.max_approver_length) {
    fail("domain-map approval approved_by is invalid");
  }
  validDate(approval.approved_at, "domain-map approval approved_at");
  const map = readVerifiedRepositoryFile(root, approval.map_path, { label: "approved domain map" });
  if (map.sha256 !== approval.map_sha256) fail("domain-map SHA-256 does not match the approval record");
  return { approval, mapSha256: map.sha256 };
}

function validateScopeReviewSidecar(root, plan) {
  const { value: sidecar } = readVerifiedRepositoryJson(root, plan.scope_review_path, { label: "scope review sidecar", maxBytes: PLAN_BOUNDS.max_scope_review_bytes });
  exactKeys(sidecar, ["schema_version", "nodes"], "scope review sidecar");
  if (sidecar.schema_version !== 1) fail("scope review sidecar schema_version must be 1");
  if (!isObject(sidecar.nodes)) fail("scope review sidecar nodes must be an object");
  const planIds = new Set(plan.nodes.map((node) => node.id));
  for (const [nodeId, record] of Object.entries(sidecar.nodes)) {
    if (!planIds.has(nodeId)) fail(`scope review sidecar must reference only plan node IDs: ${nodeId}`);
    exactKeys(record, ["rationale"], `scope review sidecar record ${nodeId}`);
    if (typeof record.rationale !== "string" || !record.rationale.trim() || record.rationale.length > PLAN_BOUNDS.max_rationale_length) {
      fail(`scope review sidecar rationale for ${nodeId} is invalid`);
    }
  }
  return sidecar;
}

// Generic plan validation over one verified repository-file snapshot: structural validation plus
// one human-attributed approval object, with conditional domain-map governance only when the
// plan-level domain_boundaries_changed signal is explicitly true and an optional exceptional-only
// ADR-035 sidecar when scope_review_path is present. No detached candidate/source/scope/archive
// hashes are consulted.
export function validatePlan(rootDir, planPath) {
  const root = path.resolve(rootDir);
  const relative = path.isAbsolute(planPath) ? path.relative(root, planPath) : planPath;
  const { bytes, sha256 } = readVerifiedRepositoryFile(root, relative, { label: "plan" });
  let plan;
  try { plan = JSON.parse(bytes); } catch (error) { fail(`plan JSON parse error: ${error.message}`); }
  validatePlanObject(plan, bytes.length);
  if (plan.domain_boundaries_changed === true) validateDomainMapApproval(root);
  if (plan.scope_review_path !== undefined) validateScopeReviewSidecar(root, plan);
  return { plan, sha: sha256 };
}
