...
1. Read the task goal, original acceptance criteria, CRAFTS plan, changed files, and verification output. For work with non-empty `security_triggers`, use the passed plan-security report only as context; it does not replace this phase's independent criteria-and-test-suite assessment.
